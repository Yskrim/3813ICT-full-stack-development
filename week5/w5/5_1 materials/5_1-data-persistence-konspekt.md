...
**Что в нём происходит.** Angular создаёт компонент и вызывает у него метод `ngOnInit()` — по имени, просто потому что такой метод в классе есть. Запись `implements OnInit` на вызов не влияет: это конструкция TypeScript, она нужна компилятору, чтобы поймать опечатку в имени хука (метод `ngOninit` без интерфейса не вызовется, и никакой ошибки не будет). Внутри метод выводит в консоль сообщение, проверяет, определён ли в браузере объект `Storage`, и если да — записывает строку `Smith` под ключом `lastname`, читает её обратно и выводит. В консоли появляются три строки: `Testing if DOM is ready`, `Storage ready` и `Smith`.

**Источники:** [Angular — Lifecycle interfaces](https://angular.dev/guide/components/lifecycle#lifecycle-interfaces) («You can **optionally** import and `implement` these interfaces to ensure that your implementation does not have any typos or misspellings») · [TypeScript Handbook — `implements` Clauses](https://www.typescriptlang.org/docs/handbook/2/classes.html#implements-clauses) («an `implements` clause is only a check… It doesn't change the type of the class or its methods at all»).
...
HTTP — протокол **без состояния** (stateless). Это значит, что каждый запрос обрабатывается сам по себе: в нём должно быть всё, что нужно серверу для ответа, а от предыдущих запросов сервер ничего не помнит. Следующий запрос от того же человека для сервера — запрос от незнакомца. Соединение при этом может оставаться открытым — в HTTP/1.1 так по умолчанию (keep-alive), — но состояние по нему всё равно не переносится: дело не в разрыве связи, а в устройстве протокола.

**Источники:** [RFC 9110 §3.3](https://www.rfc-editor.org/rfc/rfc9110#section-3.3) («each request message's semantics can be understood in isolation… a server **MUST NOT** assume that two requests on the same connection are from the same user agent») · [RFC 9112 §9.3](https://www.rfc-editor.org/rfc/rfc9112#section-9.3) («HTTP/1.1 defaults to the use of "persistent connections"»).
...
- **Текст сообщений** выводится через интерполяцию `{{ }}`: Angular экранирует значение — HTML не разбирается, и `<script>` в сообщении покажется обычным текстом. Для пользовательского текста этого достаточно, переходить на `[innerHTML]` незачем. И тем более нельзя пропускать такой текст через `bypassSecurityTrustHtml`: этот метод снимает очистку у HTML-привязок вроде `[innerHTML]` и открывает прямой путь к XSS.
- **Токен сессии**, если он есть, хранят в куке с `HttpOnly`, `Secure` и `SameSite`.

**Источники:** [Angular — Sanitization and security contexts](https://angular.dev/best-practices/security#sanitization-and-security-contexts) (HTML-контекст — «when binding to `innerHtml`»; «Interpolated content is always escaped — the HTML isn't interpreted») · [Angular — Trusting safe values](https://angular.dev/best-practices/security#trusting-safe-values) (`bypassSecurityTrust*` как способ «prevent automatic sanitization»).
